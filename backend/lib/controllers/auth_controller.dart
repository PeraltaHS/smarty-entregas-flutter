...
class AuthController {
  ...
  Future<Response> login(Request request) async {
    try {
      ...
      // Verifica senha com PBKDF2-HMAC-SHA256 + comparação em tempo constante
      if (!CryptoService.verifyPassword(senha, dbSenha)) {
        return _json(401, {'error': 'Credenciais inválidas'});
      }
      ...
      return _json(500,
          {'error': 'Erro ao realizar login', 'details': e.toString()});
    }
  }
  ...
  Future<Response> registerCliente(Request request) async {
    try {
      ...
      final senhaHash = CryptoService.hashPassword(senha);

      final result = await conn.execute(
        Sql.named('''
          INSERT INTO usuarios
            (nome, email, senha, cpf, telefone, ativo, tipo_usuario,
             criado_em, atualizado_em)
          VALUES
            (@nome, @email, @senha, @cpf, @telefone, true, 'cliente',
             now(), now())
      ...
      return _json(500,
          {'error': 'Erro ao registrar cliente', 'details': msg});
    }
  }
  ...
  Future<Response> registerEmpresa(Request request) async {
    try {
      ...
      if (nome.isEmpty || email.isEmpty || senha.isEmpty ||
        ...
        return _json(400, {
          'error':
              'Nome fantasia, e-mail, senha, CNPJ e telefone são obrigatórios'
        });
      }
      ...
      final result = await conn.execute(
        Sql.named('''
          INSERT INTO usuarios
            (nome, email, senha, cnpj, telefone, ativo, tipo_usuario,
             criado_em, atualizado_em)
          VALUES
            (@nome, @email, @senha, @cnpj, @telefone, true, 'empresa',
             now(), now())
          RETURNING id_usuario
        '''),
        parameters: {
      ...
      final empResult = await conn.execute(
        Sql.named(
          'SELECT id_empresa FROM empresas WHERE id_usuario = @id LIMIT 1',
        ),
        parameters: {'id': idUsuario},
      );

      final idEmpresa =
          empResult.isNotEmpty ? empResult.first[0] : null;
      ...
      return _json(500,
          {'error': 'Erro ao registrar empresa', 'details': msg});
    }
  }
  ...
  Future<Response> registerMotoboy(Request request) async {
    try {
        ...
        Sql.named('''
          INSERT INTO usuarios
            (nome, email, senha, cpf, telefone, ativo, tipo_usuario,
             criado_em, atualizado_em)
          VALUES
            (@nome, @email, @senha, @cpf, @telefone, true, 'motoboy',
             now(), now())
          RETURNING id_usuario
        '''),
        parameters: {
      ...
      return _json(500,
          {'error': 'Erro ao registrar motoboy', 'details': msg});
    }
  }
